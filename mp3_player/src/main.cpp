...
void playNextInSequence() {
  if (isPlayingASequence && currentActiveSequence && currentActiveSequenceIndex < currentActiveSequenceLength) {
    Serial.print("Playing sequence track: "); Serial.println(currentActiveSequence[currentActiveSequenceIndex]);
    audio.stopSong();
    delay(50);
    audio.connecttoFS(SD_MMC, currentActiveSequence[currentActiveSequenceIndex]);
    trackWasRunning = audio.isRunning(); // Update trackWasRunning for the new song
  } else {
...
void startAudioPlayback(Button& btn) {
  ...
  if (btn.is_sequence_starter) {
    ...
    // Direct play
    Serial.print("Playing direct track: "); Serial.println(btn.direct_mp3_file);
...
void setup() {
   ...
   if (!SD_MMC.begin("/sdcard",true)) { //true to use 1-bit mode to free up pins 4, 12, 13
